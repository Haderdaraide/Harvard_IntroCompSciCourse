...
char* indexes(const char* path)
{
    char pathCopiedPHP[strlen(path) + 10];
    char pathCopiedHTML[strlen(path) + 11];
    char PHPIndex[] = "index.php";
    char HTMLIndex[] = "index.html";
    char* correctPath;
    
    char* indexWithPHP = (char*) malloc(strlen(PHPIndex));
    char* indexWithHTML = (char*) malloc(strlen(HTMLIndex));
    strcpy(indexWithPHP, PHPIndex);
    strcpy(indexWithHTML, HTMLIndex);
    
    strcpy(pathCopiedPHP, path);
    strcat(pathCopiedPHP, indexWithPHP);
    
    if (access(pathCopiedPHP, R_OK) != -1)
    {
        correctPath = pathCopiedPHP;
        correctPath[strlen(pathCopiedPHP)] = '\0';
        free(indexWithPHP);
        free(indexWithHTML);
        return correctPath;
    }
    
    strcpy(pathCopiedHTML, path);
    strcat(pathCopiedHTML, indexWithHTML);
    
    if (access(pathCopiedHTML, R_OK) != -1)
    {
        correctPath = pathCopiedHTML;
        correctPath[strlen(pathCopiedHTML)] = '\0';
        free(indexWithPHP);
        free(indexWithHTML);
        return correctPath;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    //create the initial array and give it enough memory for a byte and the \0 end
    char* theData = malloc(sizeof(char) + 1);
    //this will be our counter 
    int numberOfBytes = 0;
    
    for(int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        theData[numberOfBytes] = (char) c;
        numberOfBytes++;
        theData = (char*) realloc(theData, sizeof(char)*numberOfBytes + 1);
    }
    
    //Derefence ** and get *content to be the address for the first byte 
    *content = theData;
    //point to the number of bytes that we loaded with *length
...
const char* lookup(const char* path)
{
    char* copiedStringForLookup;
    copiedStringForLookup = (char*) malloc(strlen(path) + 1);
    strcpy(copiedStringForLookup, path);
    copiedStringForLookup[strlen(copiedStringForLookup)] = '\0';
    
    char thePeriodToFind = '.';
    char* lastUsedPeriod = strrchr(copiedStringForLookup, thePeriodToFind);
    char* MIMETypeToReturn = " ";
    
    if (strcasecmp(lastUsedPeriod, ".css") == 0)
    {
        MIMETypeToReturn = "text/css";
    }
    else if (strcasecmp(lastUsedPeriod, ".html") == 0)
    {
        MIMETypeToReturn = "text/html";
    }
    else if (strcasecmp(lastUsedPeriod, ".gif") == 0)
    {
        MIMETypeToReturn = "image/gif";
    }
    else if(strcasecmp(lastUsedPeriod, ".ico") == 0)
    {
        MIMETypeToReturn = "image/x-icon";
    }
    else if(strcasecmp(lastUsedPeriod, ".jpg") == 0)
    {
        MIMETypeToReturn = "image/jpeg";
    }
    else if(strcasecmp(lastUsedPeriod, ".js") == 0)
    {
        MIMETypeToReturn = "text/javascript";
    }
    else if(strcasecmp(lastUsedPeriod, ".php") == 0)
    {
        MIMETypeToReturn = "text/x-php";
    }
    else if(strcasecmp(lastUsedPeriod, ".png") == 0)
    {
        MIMETypeToReturn = "image/png";
    }
    else
    {
        free(copiedStringForLookup);
        return NULL;
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* TheMethodCopy;
    char* request_lineCopy;
    char* HTTPCopy;
    
    TheMethodCopy = strtok(lineCopy, " ");
    request_lineCopy = strtok(NULL, " ");
    HTTPCopy = strtok(NULL, "\r\n");
    
    char method[strlen(TheMethodCopy) + 1];
    strcpy(method, TheMethodCopy);
    method[strlen(TheMethodCopy)] = '\0';
    
    char request_line[strlen(request_lineCopy) + 1];
    strcpy(request_line, request_lineCopy);
    request_line[strlen(request_lineCopy)] = '\0';
    
    char HTTP[strlen(HTTPCopy) + 1];
    strcpy(HTTP, HTTPCopy);
    HTTP[strlen(HTTPCopy)] = '\0';
    ...
    if (strstr(request_line, "?") != NULL)
    {
        ...
        {
            strcpy(query, queryIndex);
            query[strlen(queryIndex)] = '\0';
        }
    }
    else
    {
